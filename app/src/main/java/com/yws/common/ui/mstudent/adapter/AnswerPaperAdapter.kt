package com.yws.common.ui.mstudent.adapter
 ...
 * <p>update: none
 */
class AnswerPaperAdapter : Adapter<AnswerPaperAdapter.AnswerPaperViewHolder>() {
    var onClick: ((Int, AnswerPaper) -> Unit)? = null
    val data = mutableListOf<AnswerPaper>()
    private val mData: List<AnswerPaper> get() = data

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AnswerPaperViewHolder {
        return AnswerPaperViewHolder(parent.context).apply {
    ...
    override fun onBindViewHolder(holder: AnswerPaperViewHolder, position: Int) {
        holder.onBind(mData[position])
    }

    inner class AnswerPaperViewHolder(context: Context) : ViewHolder(ConstraintLayout(context)) {

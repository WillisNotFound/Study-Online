...
import com.yws.common.ui.mteacher.adapter.AnswerPaperAdapter2
 ...
 * <p>update: none
 */
class ExamMakeFragment : Fragment() {
    private val mViewModel by viewModels<ExamMakeViewModel>()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return ConstraintLayout(inflater.context).apply {
            ...
            val ansPaperAdapter = AnswerPaperAdapter2().apply {
                onClick = { position, answerPaper ->
                    if (answerPaper.status == AnswerPaperStatus.Idle) {
                        lifecycleScope.launchWhenResumed {
                            val result = ExamMakingDialog.showDialog(childFragmentManager, answerPaper)
                            if (result is AnswerPaper) {
                                data[position] = result
                                notifyItemChanged(position)
                            }
                        }
                    } else {
                        toastService.showToast("已批改")
                    }
                }

                mViewModel.ansPaperList.collectWhenResumed(lifecycleScope) {
                    data.clear()
                    data.addAll(it)
